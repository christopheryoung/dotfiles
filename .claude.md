...
On my personal machine (check for a file `~/.personal_machine`), always use
`uv` to run Python tools and scripts. E.g., `uv run pytest`, `uv run ruff
check`, `uv run python scripts/foo.py`.
